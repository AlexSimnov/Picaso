...
from files.tasks import upload_file
...
class ApiPagesURLTests(APITestCase):
    ...
    def test_upload_url_get_requests(self):
        ...
        self.assertEqual(
            response_get.status_code, HTTPStatus.METHOD_NOT_ALLOWED
            )
...
class FileModelTests(TestCase):
    def test_str(self):
        file = File.objects.create(
            file="test.txt", uploaded_at=timezone.now(), processed=True
        )
    ...
    def test_processed(self):
        file = File.objects.create(
            file="test.txt", uploaded_at=timezone.now(), processed=False
        )
